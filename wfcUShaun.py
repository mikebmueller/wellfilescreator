# wfcUShaun.py - Populates wellfiles for CPEC Upper Shaunavon wells and saves to directory
...
Possibly gather prog depth info from Stick.xls file. This must be a non-mandatory option as user isn't always given the file. 
Use if statement here

Get screen resolution and use that for easyGUI when putting well name in striplogs
...
hztl_template_path = abspath("Templates\CPEC UShaun\Lateral Template.hlg")
build_template_path = abspath("Templates\CPEC UShaun\Vertical-Build Template.slg")
tvd_template_path = abspath("Templates\CPEC UShaun\TVD-Gamma Template.slg")
build_window_name = 'STRIP.LOG - Vertical-Build Template.slg'
tvd_window_name = 'STRIP.LOG - TVD-Gamma Template.slg'
wellpath_path = abspath("Templates\CPEC UShaun\Wellpath Template.xlsx")
report_path = abspath("Templates\CPEC UShaun\Report Template.docx")
hazard_path = abspath("Templates\CPEC UShaun\Hazard Template.docx")
vertbuild_header = "Build Strip"
...
# Fix brackets for save directory if using SendKeys
save_dirFix = save_dir
if "(" and ")" in save_dirFix:
  save_dirFix = save_dirFix.replace("(", "{(}")
  save_dirFix = save_dirFix.replace(")", "{)}")
...
# Fix brackets for SendKeys in Well Name
wellNameBracket = wellInfo[0]
if "(" and ")" in wellInfo[0]:
  wellNameBracket = wellNameBracket.replace("(", "{(}")
  wellNameBracket = wellNameBracket.replace(")", "{)}")
...
if wellInfo[6] is not "":
  ...
  KB_rounded = 0
# Create .txt with wellInfo and save to directory
'''
wellTxt = open("wellinfo.txt", "w")
wellTxt.write(wellInfo)
wellTxt.close()
'''
...
def ControlExcel():
  ...
  # Add text box for wellpath title
  ...
  # Add KB depth to Reference Depths sheet
...
def ControlWord():
  ...
  report_bookmarks = ['WellNameCover1', 'WellNameCover2', 'LicenseCover', 'UWICover', 'GeoNameCover', 'Geo2Name', 'PrepForCover', 
  'WellName', 'UWI', 'PrimObj', 'License', 'Ground', 'KB', 'Rig', 'KBGeoMark', 'WellNameSurveys', 'LocationSurveys', 'RigSurveys', 
  'KBSurveys', 'WellNameSummary', 'PrimObjSum', 'RegionSum', 'WellNameEnd', 'GeoNameEnd']
  report_datalist = [wellname_first, wellname_end, 'Well License #: ' + wellInfo[1], 'UWI: ' + wellInfo[2], username, geo2name, 
  user_profile.reporting_to, wellInfo[0], wellInfo[2], wellInfo[7], wellInfo[1], wellInfo[5] + ' m', wellInfo[6] + ' m', user_profile.rig, 
  KB_rounded, wellInfo[0], wellInfo[4], user_profile.rig, KB_rounded, wellInfo[0], wellInfo[7], wellInfo[8], wellInfo[0], username]
  for i in range(24):
  ...
  # Save Report
  word.ActiveDocument.SaveAs(save_dir + wellInfo[0] + " Geological Report.docx")
...
def ControlHztl(login_title):
  ...
  # Enter info into main header form and go into Alt+i
  app[u'TMainHeaderForm'].TypeKeys(wellNameBracket + " {(}Lateral{)}" + "{TAB}" + wellInfo[4] + "{TAB}" + wellInfo[1] + 
  "{TAB}" + wellInfo[8] + "{TAB}{TAB}{TAB}{TAB}{TAB}" + wellInfo[5] + "{TAB}" + wellInfo[6] + "{TAB}{TAB}{TAB}{TAB}" 
  + wellInfo[7] +"%i", with_spaces=True)
  ...
  app[u'Other Information'].TypeKeys("{TAB}Supervision:{SPACE}" + user_profile.companyman + "{ENTER}Rig:{SPACE}"
  + user_profile.rig + "{TAB}{TAB}" + user_profile.directional + "{ENTER}DD: " + user_profile.dd1 + "{ENTER}DD: " 
  + user_profile.dd2 + "{ENTER}MWD: " + user_profile.mwd1 + "{ENTER}MWD: " + user_profile.mwd2 + "{TAB}{TAB}License:{SPACE}"
  + wellInfo[1] + "{ENTER}AFE:{SPACE}" + wellInfo[3] + "{ENTER}UWI:{SPACE}" + wellInfo[2] + "{ENTER}{ENTER}{TAB}" + "{ENTER}", with_spaces=True)
  app[u'TMainHeaderForm'].Wait('ready')
  app[u'TMainHeaderForm'].TypeKeys("%o")
  app[u'Operator'].Wait('ready')
  app[u'Operator'].TypeKeys("Crescent Point Energy Corp. {(}Prepared for{SPACE}" + user_profile.reporting_to + "{)}" 
  ...
  # Add Well name to top of logs
  app[u'TDiagramForm'].TypeKeys('{HOME}')
  app[u'TDiagramForm'].ClickInput(coords=(10,550), double=True)
  app[u'TDiagramForm'].TypeKeys(wellNameBracket + '{ESC}', with_spaces=True)
  # Add Spud, GL, and KB info underneath
  app[u'TDiagramForm'].RightClickInput(coords=(10,650))
  app[u'TDiagramForm'].TypeKeys('n')
  app[u'TDiagramForm'].ClickInput(coords=(10,650), double=True)
  app[u'TDiagramForm'].TypeKeys('G.L: ' + wellInfo[5] + ' m' + '^{ENTER}' + 'K.B.: ' + wellInfo[6] + ' m'
  + '^{ENTER}' + '^{ENTER}Spud: Month xxth, 2017 at xx hrs{ESC}', with_spaces=True)
  app[u'TDiagramForm'].Wait('ready')
  app[u'HORIZONTAL.LOG - Lateral Template.hlg'].MenuSelect("File -> Save As")
  app[u'Save Log File AsDialog'].TypeKeys(save_dirFix + wellNameBracket + " Lateral Strip.hlg{ENTER}", with_spaces=True)
  return
...
def ControlVert(login_title, window_name, template_path, log_type):
  ...
  app[u'TMainHeaderForm'].Wait('ready')
  # Enter info into main header form and go into Alt+i
  app[u'TMainHeaderForm'].TypeKeys(wellNameBracket + " {(}" + log_type + "{)}" + "{TAB}" + wellInfo[4] + "{TAB}" + wellInfo[1] + 
  "{TAB}" + wellInfo[8] + "{TAB}{TAB}{TAB}{TAB}{TAB}" + wellInfo[5] + "{TAB}" + wellInfo[6] + "{TAB}{TAB}{TAB}{TAB}" 
  + wellInfo[7] +"%i", with_spaces=True)
  app[u'Other Information'].Wait('ready')
  app[u'Other Information'].TypeKeys("{TAB}Supervision:{SPACE}" + user_profile.companyman + "{ENTER}Rig:{SPACE}" 
  ...
  app[u'TMainHeaderForm'].TypeKeys("%c")
  # Add Well name to top of logs
  app[window_name].TypeKeys('{HOME}')
  app[window_name].ClickInput(coords=(500,60), double=True)
  app[window_name].TypeKeys(wellNameBracket + '^{ENTER}' + nameTag + '{ESC}', with_spaces=True)
  # Add Spud, GL, and KB info underneath
  app[window_name].RightClickInput(coords=(500,140))
  app[window_name].TypeKeys('n')
  app[window_name].ClickInput(coords=(500,140), double=True)
  app[window_name].TypeKeys('G.L: ' + wellInfo[5] + ' m' + '^{ENTER}' + 'K.B.: ' + wellInfo[6] + ' m^{ENTER}^{ENTER}'
  + 'Spud: Month xxth, 2017 at xx hrs' + '^{ENTER}Surface Casing set at xx m MD' + '^{ENTER}Sample Point {(}xx m MD{)}: '
  + 'Month xxth, 2017 at xx hrs' + '^{ENTER}KOP {(}xx m MD{)}: Month xxth, 2017 at xx hrs'
  + '^{ENTER}Mud Up {(}xx m MD{)}: Month xxth, 2017 at xx hrs{ESC}', with_spaces=True)
  app[window_name].Wait('ready')
  app[window_name].MenuSelect("File -> Save As")
  app[u'Save Log File AsDialog'].TypeKeys(save_dirFix + wellNameBracket + " " + log_type + ".slg{ENTER}", with_spaces=True)
  return

OpenWellSight(hztl_log_path)
ControlHztl(hztl_login_title)
nameTag = 'Vertical & Build Section'
OpenWellSight(vert_log_path)
ControlVert(vert_login_title, build_window_name, build_template_path, vertbuild_header)
nameTag = 'Vertical & Build Section {(}TVD-Gamma{)}'
OpenWellSight(vert_log_path)
