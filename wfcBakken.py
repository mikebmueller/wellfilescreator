# wfcBakken.py - Populates wellfiles for CPEC Bakken wells and saves to directory
...
Gather screen resolution and use it to modify mouse coordinates
'''
...
def ControlWord():
  ...
  'WellName', 'UWI', 'License', 'Ground', 'KB', 'Rig', 'KBGeoMark', 'WellNameSurveys', 'LocationSurveys', 'RigSurveys', 
  'KBSurveys', 'WellNameSummary', 'WellNameEnd', 'GeoNameEnd']
  report_datalist = [wellname_first, wellname_end, 'Well License #: ' + wellInfo[1], 'UWI: ' + wellInfo[2], username, geo2name, 
  user_profile.reporting_to, wellInfo[0], wellInfo[2], wellInfo[1], wellInfo[5] + ' m', wellInfo[6] + ' m', user_profile.rig, 
  KB_rounded, wellInfo[0], wellInfo[4], user_profile.rig, KB_rounded, wellInfo[0], wellInfo[0], username]
  for i in range(21):
...
def ControlHztl(login_title):
  ...
  app[u'Other Information'].TypeKeys("{TAB}Supervision:{SPACE}" + user_profile.companyman + "{ENTER}Rig:{SPACE}" + user_profile.rig 
  + "{TAB}{TAB}" + user_profile.directional + "{ENTER}DD: " + user_profile.dd1 + "{ENTER}DD: " + user_profile.dd2 + "{ENTER}MWD: " 
  + user_profile.mwd1 + "{ENTER}MWD: " + user_profile.mwd2 + "{TAB}{TAB}AFE:{SPACE}" + wellInfo[3] + "{ENTER}UWI:{SPACE}" + wellInfo[2] 
  + "{TAB}" + "{ENTER}", with_spaces=True)
  app[u'TMainHeaderForm'].Wait('ready')
  ...
  app[u'Save Log File AsDialog'].TypeKeys(save_dirFix + wellNameBracket + " - Lateral.hlg{ENTER}", with_spaces=True)
  return
